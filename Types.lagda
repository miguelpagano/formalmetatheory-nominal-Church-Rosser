...
data Type : Set where
  ...
  _⟶_  : Type → Type → Type
import Context
open module M = Context (Type) (_≟ₐ_)
  renaming (begin_ to begin⊆_;_∎ to _∎⊆) public
\end{code}
...
data _⊢_∶_ (Γ : Cxt): Λ → Type → Set where
  ⊢v : {x : Atom}                     → (p∈ : x ∈ Γ)                → Γ ⊢ v x ∶ Γ ⟨ p∈ ⟩
  ⊢· : {α β : Type}{M N : Λ}       → Γ ⊢ M ∶ α ⟶ β → Γ ⊢ N ∶ α   → Γ ⊢ M · N ∶ β
  ⊢ƛ : {x : Atom}{α β : Type}{M : Λ}  → Γ ‚ x ∶ α ⊢ M ∶ β           → Γ ⊢ ƛ x M ∶ α ⟶ β
\end{code}
%</type>


\begin{code}
lemma⊢v : {x : Atom}{α : Type}{Γ : Cxt} → Γ ⊢ v x ∶ α → Σₓ (x ∈ Γ) (λ x∈Γ → Γ ⟨ x∈Γ ⟩ ≡ α)
...
%<*typeweakening#>
\begin{code}
lemmaStrengthening⊢#  : {Γ : Cxt}{M : Λ}{x : Atom}{α β : Type}
                      → x # M → Γ ‚ x ∶ β ⊢ M ∶ α → Γ ⊢ M ∶ α
...
lemma⊢α {M}
  ...
  lemmaƛ M x PMπ {Γ} {α ⟶ β} {ƛ y N} (⊢ƛ .{x} .{α} .{β} Γ,x:α⊢M:β) (∼αƛ xs ∀z∉xs,（xz）M∼（yz）N)
    ...
    z∉domΓ : z ∉l dom Γ
    z∉domΓ = ∉-++⁻ˡ (dom Γ) (∉-++⁻ʳ xs z∉xs++domΓ++fvN)
    z≢y : z ≢ y
    z≢y = ∉-∷⁼ (here refl) z∉y∷xs++domΓ++fvN
    z#N : z # N
    z#N = lemma∉fv→# (∉-++⁻ʳ (dom Γ) (∉-++⁻ʳ xs  z∉xs++domΓ++fvN))
    ...
    hi : Γ ‚ z ∶ α ⊢ （ y ∙ z ） N ∶ β
    hi = PMπ [(x ∶ z)] weakening (∀z∉xs,（xz）M∼（yz）N z (∉-++⁻ˡ xs z∉xs++domΓ++fvN))
...
lemma⊢[] {x} {N} {M}
  ...
  lemmaƛ : (P : Λ) (b : Atom) → b ∉l x ∷ fv N → P⊢[] x N P → P⊢[] x N (ƛ b P)
